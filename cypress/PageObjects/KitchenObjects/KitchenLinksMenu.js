...
LINK_MEAL_FUND = "a[title='Meal Fund']"
LINK_ACCOUNTS = "a[title='Accounts']"
clickMealFundLink()
{
  cy.get(this.LINK_MEAL_FUND).click();
}
...
clickLinkPreOrders()
{
  cy.xpath(this.LINK_PREORDERS).click({force: true});
}
clickLinkRecording()
{
  cy.xpath(this.LINK_RECORDING).click({force: true});
}
clickLinkReporting()
{
  cy.xpath(this.LINK_REPORTING).click({force: true});
}
clickLoginsLink()
{
...
clickAccountsLink()
{
  cy.get(this.LINK_ACCOUNTS).should('be.visible')
...
export default KitchenLinksMenu
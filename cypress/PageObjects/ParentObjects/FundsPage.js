class FundsPage {
  ...
  TEXTBOX_SEARCH_TEACHER = "(//input[@type='search'])[2]"
  TEXTBOX_SEARCH = "//input[@type='search']"
  ...
  selectDropdownValues() {
    cy.get(this.DROPDOWN_FUND_TYPE).select('Limited Availability').should('have.value', '9')
    cy.get(this.DROPDOWN_FUND_TYPE).select('Milk').should('have.value', '5')
    cy.get(this.DROPDOWN_FUND_TYPE).select('Trip').should('have.value', '2')
  }
  clickInactiveLink() {
  ...
  typeFundNameTeacher() {
    cy.xpath(this.TEXTBOX_SEARCH_TEACHER).type("April")
  }
  typeInvalidFundNameTeacher() {
    cy.xpath(this.TEXTBOX_SEARCH_TEACHER).clear().type("asdsa")
  }
  typeInvalidFundName() {
  ...
  clearTeacherSearchTextboxValue() {
    cy.xpath(this.TEXTBOX_SEARCH_TEACHER).focus().clear();
  }
  verifyTeacherFundPageListing() {
